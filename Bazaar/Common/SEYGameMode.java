...
// This class represents the SEY game mode, where a player will earn extra bonus points if
// the cards they buy contain the colors green, yellow, red, white, and blue.
public class SEYGameMode implements IGameMode{
    // colors needed to earn points
    private final List<PebbleColor> colors = new ArrayList<>(List.of(PebbleColor.GREEN, PebbleColor.YELLOW, PebbleColor.RED, PebbleColor.WHITE, PebbleColor.BLUE));
    ...
    public int calculateBonus(List<Card> cards) {
        ...
        if (!colorFlags.containsValue(false)) {
            return Constants.SEY_BONUS_POINTS;
        }
